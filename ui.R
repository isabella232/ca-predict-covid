...
ui <- navbarPage(
    ...
    title = "California COVID Assessment Tool",
    id = "navbar",
    ...
    header = tagList( shinyWidgets::useShinydashboard(),
                      tags$head(
                          ...
                          HTML(
                              "<script async src='https://www.googletagmanager.com/gtag/js?id=UA-170240821-1'></script>"
                          ),
             ...
             fluidPage(
                ...
                fluidRow(
                      ...
                      column(4,
                             ...
                             h4(paste0("Beta Version | Released ",date_updated)),
                     ...
                     column(4,
                            # img(src = 'cdph.png', align = "right", width = '50%'),
                            img(src = 'CDPHLogo.gif', align = "right", height = '100px'),
                            br()
...
#### Nowcast Page ####
    tabPanel( "Nowcasts",
              fluidPage(
                 h2("Current R-effective in California"),
                 ...
                 hr(),
                        ...
                        fluidRow(
                            column(3,
                                ...
                                h4("Latest Average R-effective is: "),
                                uiOutput("cnty.mean.rt.box", width = 12),
                     ...
                     fluidRow(
                       column(6,
                         downloadButton("dlRt.cnty", "Download R-eff for Counties"),
                            h4("Latest R-effective in California Counties"),
                            h6("Using estimates and uncertainty intervals from COVID Act Now, the following graph presents the averaged R-effective over the last 7 days by county; (<1 decreasing spread, >1 increasing spread)"),
                            plotlyOutput("rt.dot.plot")
                        ),
                       column(6,
                              downloadButton("dlRt.cnty.map", "Download R-eff for Counties"),
                              h4("Statewide Map of R-effective by County"),
                              h6("Using estimates and uncertainty intervals from COVID Act Now, the following map presents the averaged R-effective over the last 7 days by county; (<1 decreasing spread, >1 increasing spread)"),
                              ggiraphOutput("reff_county_map", height = "400px")
                       ) # end column
...
#### Forecasts Page ####
    tabPanel( "Forecasts",
             fluidPage(
                       ...
                       h2("Short-term COVID-19 Forecasts in California"),
                       ...
                       fluidRow(
                               column(3,
                                      ...
                                      column(9,
                                            ...
                                            h6("The black box (left) represents the current number of hospitalized COVID patients in California. The blue box 
                                                represents the average forecasted number of hospitalized patients at the 30 day mark based on models for California." ), 
                                            plotlyOutput("hosp.proj.plot", height = "100%")
                                      )

                      ), #End of fluidRow
                      hr(),
                      h4("California County Hospitalization Forecasts"),
                      ...
                      fluidRow(
                          ...
                          column(9,
                                 ...
                                 h6("The black box (left) represents the current number of total COVID deaths in California. The blue box 
                                                represents the average forecasted number of total deaths at the 30 day mark based on models for California." ), 
                                 plotlyOutput("cdeath.proj.plot", height = "100%")
                          )
                          
                      ), #End of fluidRow
                      hr(),
                      h4("California County Death Forecasts"),
                      ...
                      tabPanel("Interactive",
                               fluidRow(
                                   column(3,
                                          ...
                                          conditionalPanel(condition = "input.include_JHU_UKKC.includes('IHME_sts') & 
                                                                        input.county_ts == 'California'",
                                          ...
                                          conditionalPanel(condition = "(input.include_JHU_UKKC.includes('rand.3.1') |
                                                                         input.include_JHU_UKKC.includes('rand.2.2') |
                                                                         input.include_JHU_UKKC.includes('rand.1.3') ) & 
                                                                         input.county_ts == 'California'",
                                                           h5("RAND Model Options:"),
                                                           switchInput("RAND.iqr", label = "RAND Interval", value = FALSE,
                                                                       onStatus = "success", offStatus = "danger",
                                                                       labelWidth = "150px",
                                                                       inline = TRUE, size = 'normal', width = "auto") 
                                          ),
...
#### Data Uploads ####
    tabPanel("Upload Data",
        fluidPage(
             fluidRow(
                 column(4,
                        h3("Nowcast and Forecast Uploads"),
                        h4("If you have a data template already, skip to Step 4."),
                        h4("STEP 1. Select geography for template."),
                        selectInput(
                            inputId = "county_upload",
                            label = "Select geography:",
                            choices = cnty.list,
                            selected = "California"
                        ),
                        h4("STEP 2. Generate and download the county template."),
                        downloadButton("c_template", "Download data template"),
                        h4("STEP 3. Use the template to enter data."),
                        HTML("<p><b>INSTRUCTIONS:</b> For the dates generated, replace 'NA' with the 
                              data values you wish to upload to CalCAT. Save the file as a .csv file.</p>"),
                        h4("STEP 4. Upload the completed template."),
                        fileInput('file1', 'Upload Template:',
                                  accept = c(
                                      'text/csv',
                        ...
                        h4("STEP 5. Please inspect uploaded data."),
                        HTML("<p><b>NOTE:</b> If you encounter an error, browse for the corrected file and re-upload.</p>"),
                        #textOutput("datacheck"),
                        conditionalPanel(condition = "output.datacheck != false" ,
                            h4("STEP 6. Approve data for use by CalCAT."),
                            awesomeCheckbox(
                                inputId = "approve_upload",
                                label = "Approve uploaded data?", 
                                value = FALSE
                            ),
                            awesomeCheckbox(
                                inputId = "include_ensemble",
                                label = "Include data in ensemble estimates?", 
                                value = FALSE
                            )
                        )
                        #uiOutput("data_check")
             ...
             hr(),
             h3("Scenarios Uploads coming soon"),
    ...
    ), #End of tabPanel for Data Uploads
...
#### Technical Notes ####
    tabPanel("Technical Notes",
             fluidPage(
                fluidRow(
                 ...
                 column(4,
                        h1("Scenarios"),
                        h4("Johns Hopkins University Infectious Disease Dynamics Group"),
                        HTML("<a href = 'http://www.iddynamics.jhsph.edu/' target='_blank'>http://www.iddynamics.jhsph.edu/</a><p>The Johns Hopkins model is a location-stratified SEIR model, implemented via the monte carlo simulation with 1000 simulation paths per scenario.  Parameters such as R0, infectious period and IFR are model inputs.  Different scenarios parameterize future values of R-effective.</p>"),
                        h6("General Model Parameters:"),
                        HTML("<ul>
                    <li>Mean incubation period: 5.2 days</li>
                    <li>Infectious period: ranges from 2.6 to 6 days</lI>
                    <li>Unimitgated R0: 2-3</lI>
                    <li>Infection Fatality Rate (IFR): 1%</lI>
                    <li>Hospitalized patients who will die: 10%</lI>
                    <li>Hospitalizations admitted to the ICU: 32%</lI>
                    <li>ICU admissions that are ventilated: 15%</lI>
                    </ul>"),
                        p(),
                        HTML("<a href = 'https://github.com/HopkinsIDD/COVIDScenarioPipeline' target='_blank'>public repository</a>"),
                        hr(),
    ...
    header.append('<div style=\"float:right;color:white\"><h5>Beta Version | Released ",date_updated,"</h5></div>');
    console.log(header)")))  
